...
// console.log('双栏布局左侧路由', authStore.showMenuList);

// 动态绑定左侧菜单animate动画
const menuAnimate = ref(settings.menuAnimate);
// 隐藏静态路由中isHide == '1'的数据
const menuList = computed(() => authStore.showMenuList.filter((item) => item.meta.isHide === '1'));
    ...
    <div class="layout-column">
      <el-scrollbar>
        <div
          ...
          :class="{
            'is-active':
              columnActive === item.path || `/${columnActive.split('/')[1]}` === item.path
          }"
